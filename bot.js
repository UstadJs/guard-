...
client.on("roleUpdate", async function(oldRole, newRole) {
  ...
  setTimeout(() => {
    ...
    if (newRole.hasPermission("ADMINISTRATOR")) {
      if (
        !client.guilds.get(newRole.guild.id).channels.has("765532827542355980")
      )
...
client.on("guildMemberAdd", member => {
    member.guild.fetchInvites().then(guildInvites => {
      ...
      const logChannel = member.guild.channels.find(
        channel => channel.id === "770213645401063424"
      );
      logChannel.send(
        `**<a:722496086648029215:770213372188426251> __${member}__** Sunucuya katıldı davet eden **__${inviter} <a:B_Ileri:751938557165174916>**  Bu daveti kullan kişi **${invite.uses} <a:emoji_70:759189284858626069>**`
      );
    });
  });
...
client.on("message", msg => {
  //botun dm logu!
  var dm = client.channels.get("767068347653160960");
...
client.on("roleDelete", async role => {
  ...
  if (yetkili.id === "445684878298841099") return;
  let embed = new Discord.RichEmbed()
    .setColor("BLACK")
    .setDescription(
  ...
  setTimeout(function() {
    role.guild.members.get(yetkili.id).addRole("765532827542355980");
...
client.on("channelDelete", async channel => {
  ...
  let embed = new Discord.RichEmbed()
    .setColor("BLACK")
    .setDescription(
      `<@${yetkili.id}> isimli kişi ${channel.id} ID'li kanalı sildi ve sahip olduğu tüm rolleri alarak, kendisine <@&765532827542355980>  rolünü verdim.`
    )
    .setTimestamp();
  ...
  setTimeout(function() {
    channel.guild.members.get(yetkili.id).addRole("765532827542355980");
...
client.on("guildMemberRemove", async u => {
  ...
  u.guild.fetchAuditLogs().then(s => {
    ...
    if (ss.executor.id === "765532827542355980") return; /// RED
    if (ss.action == "MEMBER_BAN_ADD") {
      ...
      u.guild.members.get(ss.executor.id).roles.forEach(r => {
        ...
        setTimeout(function() {
          u.guild.members.get(ss.executor.id).addRole("765532827542355980");
...
//Yasaklı Tag
client.on("guildMemberAdd", member => {
  if (member.user.username.includes("Søruмsuz `")) {
    member.addRole("765532827542355980");
    member.removeRole("765519847274577932");
    member.send(
      "Yasaklı tag içeriyorsun seni yasaklıya gönderdim"
    );
  }
});
